...
describe('deleteHomeLocationFromUser', () => {
  test('should delete only the specified homeLocation from user.homeLocations', async (done) => {
    expect.assertions(3);

    //add a second homeLocation to user
    resetSeedData();
    await addHomeLocationToUser(user, getSeedHomeLocation()).then(({ err, res }) => {
      user = res;
    });

    deleteHomeLocationFromUser(user, homeLocation).then(async ({ err, res }) => {
